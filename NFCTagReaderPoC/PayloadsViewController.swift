...
class PayloadsViewController: UIViewController, UITextFieldDelegate, PKAddPassesViewControllerDelegate {
     ...
     @IBAction func showGeneratedPass(_ sender: Any) {
          
          activityIndicator.isHidden = false
          activityIndicator.startAnimating()
          
          if let url = URL(string: "https://mpass-poc.s3.us-west-1.amazonaws.com/Generic.pkpass") {
               let task = URLSession.shared.dataTask(with: url) { data, response, error in
                    ...
                    DispatchQueue.main.async { /// execute on main thread
                         ...
                         if let pkpass = try? PKPass(data: data) {
                              ...
                              addPKPassViewController.delegate = self
                              addPKPassViewController.modalPresentationStyle = .pageSheet
                              self.present(addPKPassViewController, animated: true, completion: nil)
                         }
                    }
               }
          ...
          //let pkpassUrl = URL(string: "https://mpass-poc.s3.us-west-1.amazonaws.com/Generic.pkpass")
          ...
          /*
           ...
           if let data = try? Data(contentsOf: passPath, options: .mappedIfSafe) {
           if let pkpass = try? PKPass(data: data) {
           guard let addPKPassViewController = PKAddPassesViewController(pass: pkpass) else {
           return
           }
           //                    if let pkpassURL = pkpass.passURL {
